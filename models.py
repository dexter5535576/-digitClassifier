...
class PerceptronModel(object):
    ...
    def train(self, dataset):
        ...
        while training_accuracy < 1:
            numOfCorrectSamples = 0
            numOfSamples = 0
            for x, y in dataset.iterate_once(batch_size):
                x_prediction = self.get_prediction(x)
                y_expection = nn.as_scalar(y)
                if x_prediction != y_expection:
                    self.w.update(x, y_expection)
                else:
                    numOfCorrectSamples += 1
                numOfSamples += 1
            training_accuracy = numOfCorrectSamples / numOfSamples
...
class RegressionModel(object):
    ...
    def __init__(self):
        ...
        self.w2 = nn.Parameter(200, 400)
        self.b2 = nn.Parameter(1, 400)
        self.w3 = nn.Parameter(400, 1)
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        for x in xs:
            z = nn.ReLU(nn.AddBias(nn.Linear(nn.Add(z, nn.Linear(x, self.w1)), self.w2), self.b2))
        z = nn.AddBias(nn.Linear(z, self.w3), self.b3)
        return z
